...
function getTranslationsJs(file, translations) {
        ...
        (0, traverse_1.default)(babelParsed, {
            enter: (p) => {
                ...
                if (((0, types_1.isNewExpression)(p.node) || (0, types_1.isCallExpression)(p.node)) &&
                    ...
                    const tKey = (0, common_1.getTKey)(p.node.arguments[0]);
                    ...
                    if ((0, types_1.isIdentifier)(p.node.callee) && p.node.callee.name === '_t') {
                        try {
                            const rawValue = lodash_1.default.get(translations, (0, common_1.getPath)(tKey));
                            const englishValue = typeof rawValue === "string" ? rawValue : rawValue === null || rawValue === void 0 ? void 0 : rawValue.other;
                            if (englishValue) {
                                ...
                                if (p.node.arguments.length > 2 && (0, types_1.isObjectExpression)(p.node.arguments[2])) {
                                    ...
                                    for (const prop of tagMap.properties || []) {
                                        if ((0, types_1.isObjectProperty)(prop) && ((0, types_1.isStringLiteral)(prop.key) || (0, types_1.isIdentifier)(prop.key))) {
...
const inputTranslationsRaw = (0, common_1.getTranslations)();
...
for (const tr of translatables) {
    const path = (0, common_1.getPath)(tr);
    if (lodash_1.default.get(inputTranslationsRaw, path)) {
        lodash_1.default.set(trObj, path, lodash_1.default.get(inputTranslationsRaw, path));
    }
    else if (!plurals.has(tr)) {
        lodash_1.default.set(trObj, path, tr);
    }
    else {
        lodash_1.default.set(trObj, path, {
            "other": tr,
            "one": tr,
        });
    }
}
(0, common_1.putTranslations)(trObj);
console.log();
console.log(`Wrote ${translatables.size} strings to ${common_1.OUTPUT_FILE}`);
