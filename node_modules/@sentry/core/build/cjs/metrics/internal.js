...
function _INTERNAL_captureMetric(beforeMetric, options) {
  ...
  const { release, environment, _experiments } = client$1.getOptions();
...
('Replay');
  ...
  debugLogger.consoleSandbox(() => {
    ...
    debugBuild.DEBUG_BUILD && console.log('[Metric]', serializedMetric);
  });

  captureSerializedMetric(client$1, serializedMetric);

  client$1.emit('afterCaptureMetric', metric);
}
